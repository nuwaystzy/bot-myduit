import { sendMessage, editMessageText, answerCallbackQuery, setMyCommands, sendDocument, setChatMenuButton } from './telegram.js';
...
const getHelpText = () => `📖 <b>Panduan Cara Pakai Bot</b>\n\n` +
  `<b>1. Catat Pengeluaran Gunakan  (-)</b>\n<i>" - makan siang 15k "</i> atau <i>" -10rb ngopi "</i>\n\n` +
  `<b>2. Catat Pemasukan Gunakan (+)</b>\n<i>" + gaji 2jt "</i> atau <i>" + 100 disawer "</i>\n\n` +
  `<b>3. Transaksi Crypto:</b>\n<i>"buy btc 100rb"</i> atau <i>"sell eth 0.5"</i> (Harga dicari otomatis)\n\n` +
...
async function handleMessage(message) {
  ...
  if (text.startsWith('/start')) {
    ...
    // Sinkronisasi tombol menu biru di pojok kiri bawah secara otomatis
    await setChatMenuButton(chatId, {
      type: 'web_app',
      text: 'Dashboard',
      web_app: { url: 'https://bot-myduit.vercel.app/?v=1.3' }
    }).catch(console.error);
  ...
  if (text.startsWith('/help')) {
    return sendDashboard(chatId, getHelpText());
  }

  if (text.startsWith('/editlast')) {
    const rawParams = text.substring(9).trim();
    const mockTx = parseTransaction(rawParams);
    if (!mockTx || (mockTx.type !== 'income' && mockTx.type !== 'expense')) {
        return sendMessage(chatId, '❌ <b>Format salah!</b>\nGunakan: <code>/editlast [catatan] [nominal]</code>\nContoh: <code>/editlast makan siang 20k</code>');
    }
    ...
    try {
        ...
        if (findError || !lastTx) return sendMessage(chatId, '❌ Belum ada transaksi kas yang bisa diedit.');

        const { error: updateError } = await supabase
            .from('transactions')
            .update({
                amount_rp: mockTx.amount_rp,
                note: mockTx.note,
        ...
        return sendMessage(chatId, `✅ <b>Transaksi terakhir diperbarui!</b>\n\n📝 ${mockTx.note}\n💰 Nominal: ${formatRp(mockTx.amount_rp)}`);
    } catch (err) {
        return sendMessage(chatId, `❌ <b>Gagal edit:</b> ${err.message}`);
    }
  }
  ...
  if (transaction) {
    try {
      ...
      // Tombol inline untuk edit cepat di Web App
      const webAppUrl = 'https://bot-myduit.vercel.app/?v=1.3';
      const inlineKeyboard = {
        inline_keyboard: [
          [{ text: '✏️ Edit di App', web_app: { url: `${webAppUrl}#edit=${transaction.id || ''}` } }]
        ]
      };
...
async function handleCallback(callback) {
  ...
  } else if (data === 'dash_help') {
    return editDashboard(chatId, messageId, getHelpText());
  ...
  } else if (data === 'dash_settings' || data === 'cmd_settings') {
    return editMessageText(chatId, messageId, '⚙️ <b>Pengaturan Akun</b>\n\nZona Waktu: Asia/Jakarta\nCurrency: IDR', {
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔔 Pengaturan Notifikasi', web_app: { url: 'https://bot-myduit.vercel.app/' } }],
          [{ text: '🔴 Reset Semua Data', callback_data: 'reset_confirm' }],
          [{ text: '⬅️ Kembali ke Menu', callback_data: 'dash_main' }]
        ]
      }
    });
