...
async function handleMessage(message) {
  ...
  if (transaction) {
    try {
      ...
      // Auto-fetch price if crypto and amount_rp is 0
      let livePriceNote = '';
      if (['buy', 'sell', 'trade'].includes(transaction.type) && transaction.amount_rp === 0) {
        ...
        if (livePrice > 0) {
          ...
          livePriceNote = ` (Harga Live: ${formatRp(livePrice)})`;
        } else {
          throw new Error(`Gagal mendapatkan harga live untuk ${transaction.asset}. Silakan masukkan nominal manual.`);
        }
      }
