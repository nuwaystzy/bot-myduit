...
export async function getCryptoPrice(asset) {
  ...
  try {
    ...
    throw new Error('CoinGecko: Data format mismatch');
  } catch (err) {
    lastError = `Gecko: ${err.message}`;
    console.warn(lastError);
  }

  // 2. Try Indodax (Secondary)
  try {
    ...
    console.warn(`Indodax failed: ${err.message}`);
  }
  ...
  try {
    const { data: cached } = await supabase
      ...
      .eq('asset', symbol)
      .single();
    if (cached) return { price: parseFloat(cached.price_rp), source: 'Database Cache' };
  } catch (err) {
    console.error(`Final cache fallback failed for ${symbol}`);
  }

  return { price: 0, error: lastError };
}
